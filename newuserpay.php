...
ob_start();
    ...
    //   $em=$_GET['useremail'];
      ...
      $hat = "SELECT * from `doctors` WHERE `name`= '$dc'";
     ...
     <div class="mainscreen">
            ...
            <div class="rightside">
              <form method="POST">
                ...
                <div class="row">
                <p>Email</p>
                <input type="text" class="inputbox" name="useremail" required  value=<?php echo($row['email']) ?>/></div>
                ...
                <div class="row">
                    <p>Card Number</p>
                    <input type="tel" name="cardno" class="inputbox" id="card_number" min="1" max="16" title="Enter 16 digits" placeholder="Enter 16-digit Card Number" value="<?php if(empty($cardno)){}else{echo $cardno;}; ?>">
...
<?php
$ha = "SELECT * from `doctors` WHERE `name`= '$dc'";
$dot = mysqli_query($mysqli,$ha); 
      foreach($dot as $rows) {
?>
                    <input type="text" class="inputbox" name="amount" id="amount"  value=<?php echo($rows['price']) ?>>
                </div><?php } ?>

                <button type="submit" name="pay" class="button">CheckOut</button>
<a href="homedon.php">Return to home page</a>
...
//  $eventname = $_GET['eventname'];
 ...
 $sendd = "INSERT INTO `payment` (`transaction_id`,`cardno`,`cardtype`,`date`,`amount`,`expdate`,`cvv`,`useremail`,`userphone`,`session_event`,`usertype`) 
 VALUES ('$trans_id','$cardno','$cardtype','$dat','$amount','$expdate','$cvv','$em','$userphone','session','existing user')";
$payss = mysqli_query($mysqli,$sendd);

echo '<script>alert("Doctor will send massage to your email with appointment")</script>';
// header('location: homedon.php');
